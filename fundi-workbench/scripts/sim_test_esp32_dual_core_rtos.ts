/*
  Headless ESP32 compile test: ESP32 Dual-Core RTOS Hub

  Verifies:
  - Code compiles successfully for wokwi-esp32-devkit-v1
  - Uses xTaskCreatePinnedToCore, xQueueCreate, analogReadResolution — ESP32-only APIs

  Usage:
    BACKEND_URL=http://localhost:8000 npm run sim:test:esp32-dual-core-rtos
*/
...
async function main(): Promise<void> {
  ...
  const code = loadFeaturedProjectCode('esp32-dual-core-rtos-hub');

  console.log('Compiling ESP32 Dual-Core RTOS Hub...');
  ...
  console.log('PASS: ESP32 Dual-Core RTOS Hub compiles successfully.');
}
